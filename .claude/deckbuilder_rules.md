...
Shattered Pixel Dungeon에 추가된 슬레이 더 스파이어 스타일 카드 배틀 모드.
...
카드 배틀 모드는 아직 베타 버전이다. 정식 버전 출시 시 언제든 되돌릴 수 있도록 아래 제한은 명시적인 베타 플래그/분기로 유지한다.

- `WndDungeonMode`의 `DECKBUILDER_BETA_LIMITS`가 켜져 있는 동안 카드 배틀 모드와 카드 배틀 튜토리얼은 `WARRIOR`, `HUNTRESS`만 시작할 수 있다.
- 다른 직업으로 카드 배틀 모드를 시작하려 하면 베타 안내 메시지를 표시하고 시작하지 않는다.
- `DeckBattleScene`의 `DECKBUILDER_BETA_LIMITS`가 켜져 있는 동안 17층 보스를 처치하면 보상/다음 막 진행 대신 베타 완료 메시지를 표시한다.
- 해당 베타 완료 메시지를 클릭하면 아래 임시 엔딩 흐름으로 이동한다.
...
정식 버전에서 이후 막을 열 때는 이 베타 제한 플래그와 17층 보스 분기를 제거하거나 비활성화한다.
...
| `TRANSMOGRIFIER = 2` | 변환 성소 | 기본 | 덱에서 카드 1장 선택 → 무작위 카드로 변환 |
| `GOLDEN_SHRINE = 3` | 황금 성소 | 골드 50 이상 | 100골드 또는 275골드+부식의 저주 |
