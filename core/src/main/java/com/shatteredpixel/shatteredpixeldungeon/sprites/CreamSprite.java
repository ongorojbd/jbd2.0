...
public class CreamSprite extends MobSprite {
    ...
    public CreamSprite() {
        ...
        if (spray != null) {
            ...
            spray.pour(CreamParticle.FACTORY, 0.04f);
            spray.on = false;
        }
    }
        ...
        if (warnDist == 0){
            ...
            for (int i = 0; i < Dungeon.level.length(); i++){
                if (ch.fieldOfView != null && ch.fieldOfView[i]
                    ...
                    e.pour(CreamParticle.FACTORY, 0.04f);
    ...
    public static class CreamParticle extends PixelParticle.Shrinking {

        public static final Emitter.Factory FACTORY = new Factory() {
            @Override
            public void emit( Emitter emitter, int index, float x, float y ) {
                ((CreamParticle)emitter.recycle( CreamParticle.class )).reset( x, y );
            }
        };

        public CreamParticle() {
            super();
