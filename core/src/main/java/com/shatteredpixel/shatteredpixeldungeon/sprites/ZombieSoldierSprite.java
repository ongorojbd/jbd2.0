...
public class ZombieSoldierSprite extends MobSprite {

    private int cellToAttack;

    public ZombieSoldierSprite() {
        super();

        texture( Assets.Sprites.ZOMBIESOL );
    ...
    public void onComplete( Animation anim ) {
        if (anim == zap) {
            ...
            emitter().burst( FlameParticle.FACTORY, 4);
            emitter().burst( SmokeParticle.FACTORY, 4 );

            ((MissileSprite)parent.recycle( MissileSprite.class )).
                    reset( this, cellToAttack, new ZombieSoldierSprite.SoldierShot(), new Callback() {
