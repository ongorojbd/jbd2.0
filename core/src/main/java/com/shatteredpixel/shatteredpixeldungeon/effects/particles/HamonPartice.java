...
public class HamonPartice extends PixelParticle.Shrinking {

    public static final Emitter.Factory MISSILE = new Factory() {
        @Override
        public void emit( Emitter emitter, int index, float x, float y ) {
            ((HamonPartice)emitter.recycle( HamonPartice.class )).reset( x, y );
        }
    };
    ...
    public static final Emitter.Factory CURSE = new Factory() {
        @Override
        public void emit( Emitter emitter, int index, float x, float y ) {
            ((HamonPartice)emitter.recycle( HamonPartice.class )).resetCurse( x, y );
        }
    };
    ...
    public static final Emitter.Factory UP = new Factory() {
        @Override
        public void emit( Emitter emitter, int index, float x, float y ) {
            ((HamonPartice)emitter.recycle( HamonPartice.class )).resetUp( x, y );
        }
    };
    ...
    public void update() {
        ...
        color( ColorMath.interpolate( 0xFFCC00, 0xFF9900, p ) );
