...
public class Kars extends Mob {
                    ...
                    if (PathFinder.distance[i] < Integer.MAX_VALUE) {
                        ...
                        if (ch != null && !isHit) {
                            if ((ch.alignment != alignment || ch instanceof Bee)) {
                                ch.damage(Random.NormalIntRange(13, 15), new Kars.Volcano());
    ...
    private boolean FireBlast() {
        ...
        if (!Dungeon.hero.rooted) {
            ...
            for (Char ch : affected) {
                ch.damage(Random.NormalIntRange(25, 30), new Kars.Blast());
    ...
    public static void spawn(CavesLevel level) {
        // 독립적인 시드 오프셋을 사용
        Random.pushGenerator(Dungeon.seedCurDepth() + 999992L);
        int max = 4;
