...
public class Supression2 extends Mob {
		...
		public boolean act( boolean enemyInFOV, boolean justAlerted ) {
			...
			if (leapPos != -1){
				...
				sprite.jump(pos, leapPos, new Callback() {
					...
					public void call() {
						...
						if (endPos != leapPos){
							Actor.addDelayed(new Pushing(Supression2.this, leapPos, endPos), -1);
						}

						pos = endPos;
						leapPos = -1;
						sprite.idle();
						Dungeon.level.occupyCell(Supression2.this);
						next();
					}
				});
				return false;
			...
			if (enemyInFOV && !isCharmedBy( enemy ) && canAttack( enemy )) {
				...
				if (enemyInFOV) {
					...
					state = WANDERING;
					target = Dungeon.level.randomDestination( Supression2.this );
					return true;
				}
	...
	public void die( Object cause ) {
		...
		Dungeon.level.drop( new Starflower.Seed(), pos ).sprite.drop( pos );
