...
public class CrystalSpire extends Mob {
	...
	protected boolean act() {
		...
		if (!targetedCells.isEmpty()){
			...
			sprite.showStatus(CharSprite.WARNING, Messages.get(CrystalSpire.class, "3"));
		...
		if (c == Dungeon.hero){
			...
			Dungeon.hero.sprite.attack(pos, new Callback() {
				...
				public void call() {
					...
					if (hits == 1){
						yell( Messages.get(CrystalSpire.class, "0"));
						GLog.w(Messages.get(CrystalSpire.class, "1"));
						PixelScene.shake( 1, 0.7f );
					} else if (hits >= 3) {
						...
						if (hits == 3){
							...
							PixelScene.shake( 3, 0.7f );
							yell( Messages.get(CrystalSpire.class, "alert"));
							GLog.w(Messages.get(CrystalSpire.class, "alert2"));
