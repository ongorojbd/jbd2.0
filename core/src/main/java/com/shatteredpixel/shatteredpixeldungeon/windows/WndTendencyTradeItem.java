...
public class WndTendencyTradeItem extends WndInfoItem {
    ...
    //selling
    public WndTendencyTradeItem( final Item item, WndBag owner ) {
        ...
        //find the TendencyShopkeeper in the current level
        TendencyShopkeeper shop = null;
        for (Char ch : Actor.chars()){
            if (ch instanceof TendencyShopkeeper){
                shop = (TendencyShopkeeper) ch;
                break;
            }
        }
        final TendencyShopkeeper finalShop = shop;

        if (item.quantity() == 1) {

            int price = item.value() / 2; // 아이템 가치의 절반으로 판매
            ...
            int priceAll = item.value() / 2; // 아이템 가치의 절반으로 판매
    ...
    //buying
    public WndTendencyTradeItem( final Heap heap ) {
        ...
        final int price = TendencyShopkeeper.sellPrice( item );
                            ...
                            protected void onSelect(int index) {
                                ...
                                if (index == 0){
                                    if (thievery.steal(item)) {
                                        Hero hero = Dungeon.hero;
                                        Item item = heap.pickUp();
                                        WndTendencyTradeItem.this.hide();
                                        ...
                                        for (Mob mob : Dungeon.level.mobs) {
                                            if (mob instanceof TendencyShopkeeper) {
                                                mob.yell(Messages.get(mob, "thief"));
                                                ((TendencyShopkeeper) mob).flee();
                                                break;
                                            }
                                        }
                                        WndTendencyTradeItem.this.hide();
    ...
    public void hide() {
        ...
        if (selling) TendencyShopkeeper.sell();
    }
    ...
    public static void sell( Item item, TendencyShopkeeper shop ) {
        ...
        if (shop != null){
            ...
            while (shop.buybackItems.size() > TendencyShopkeeper.MAX_BUYBACK_HISTORY){
    ...
    public static void sellOne( Item item, TendencyShopkeeper shop ) {
    ...
    private void buy( Heap heap ) {
        ...
        int price = TendencyShopkeeper.sellPrice( item );
        Dungeon.gold -= price;
