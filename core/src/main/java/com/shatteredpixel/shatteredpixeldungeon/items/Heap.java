	...
	public String title(){
		switch(type){
			case FOR_SALE:
				Item i = peek();
				if (size() == 1) {
					if (Dungeon.tendencylevel) return Messages.get(this, "for_sale", TendencyShopkeeper.sellPrice(i), i.title());
					else return Messages.get(this, "for_sale", Shopkeeper.sellPrice(i), i.title());
				} else {
					return i.title();
				}
			case CHEST:
