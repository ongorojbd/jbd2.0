...
public class Highway extends Spell {

    {
        image = ItemSpriteSheet.HIGHWAY;
    }
    ...
    protected void onCast(Hero hero) {
        ...
        for (Mob mob : Dungeon.level.mobs.toArray( new Mob[0] )) {
            if (!mob.properties().contains(Char.Property.BOSS) && !mob.properties().contains(Char.Property.MINIBOSS)
                    && Dungeon.level.heroFOV[mob.pos] && Sacrifice < 1) {
        ...
        GLog.n(Messages.get(this, "w"));
        Sample.INSTANCE.play(Assets.Sounds.LIGHTNING, 1.5f, 0.66f);
